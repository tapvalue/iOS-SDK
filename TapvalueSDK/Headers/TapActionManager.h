...
/**
 * Checks if the notification is a remote notification push by the tapvalue servers.
 *
 * @param userInfo The notification dictionnary
 *
 * @return Flag indicating if the notification is a TapSilentNotification.
 */
- (BOOL) isRemoteTapNotification:(NSDictionary *) userInfo;
...
/**
 * Executes the action for the given silent notification informations.
 *
 * @param info The notification informations to process.
 */
- (void) processActionForRemoteTapNotificationInfo:(NSDictionary *) info fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler;
